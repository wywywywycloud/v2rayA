#!/bin/sh
# Add the signed Levin feed; install the fork later through LuCI Software.
set -eu
umask 077
...
feed_url='https://raw.githubusercontent.com/wywywywycloud/v2rayA-current/openwrt-feed/openwrt-24.10/levin/aarch64_cortex-a53'
...
work_dir=$(mktemp -d /tmp/v2raya-levin.XXXXXX)
...
awk '$2 != "v2raya_levin" && $2 != "v2raya_current" && $2 != "v2raya_fork"' /etc/opkg/customfeeds.conf > "$work_dir/customfeeds.conf"
printf 'src/gz v2raya_levin %s\n' "$feed_url" >> "$work_dir/customfeeds.conf"
if ! cmp -s "$work_dir/customfeeds.before" "$work_dir/customfeeds.conf"; then
    cp "$work_dir/customfeeds.before" /etc/opkg/customfeeds.conf.before-levin
    cat "$work_dir/customfeeds.conf" > /etc/opkg/customfeeds.conf
fi
rm -f /var/opkg-lists/v2raya_current /var/opkg-lists/v2raya_fork
opkg update
echo 'Feed added. In LuCI: System > Software > filter levin > Install luci-app-v2raya-levin.'
